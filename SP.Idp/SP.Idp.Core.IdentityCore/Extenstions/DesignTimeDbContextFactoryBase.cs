...
namespace SP.Idp.Core.IdentityCore.Extenstions
{
    ...
    IDesignTimeDbContextFactory<TContext> where TContext : DbContext
    {
        ...
        private TContext Create(string basePath, string environmentName, string connectionStringName, string migrationsAssemblyName)
        {
            var builder = new ConfigurationBuilder()
                .SetBasePath(basePath)                     //需要 Microsoft.Extensions.Configuration.FileExtensions包
                .AddJsonFile("appsettings.json")           //需要 Microsoft.Extensions.Configuration.Json包
                .AddJsonFile($"appsettings.{environmentName}.json", true) 
                .AddEnvironmentVariables();                //需要 Microsoft.Extensions.Configuration.EnvironmentVariables包
